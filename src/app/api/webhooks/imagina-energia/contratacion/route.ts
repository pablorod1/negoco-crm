...
import {
  ...
  processImaginaContractCallback,
  tenantFromHost,
...
export async function POST(request: Request) {
  ...
  const parsed = ImaginaContractCallbackSchema.safeParse(payload);
  if (!parsed.success) {
    return NextResponse.json(
      {
        success: false,
  ...
  const event = await recordWebhookEvent(db, {
    ...
    imaginaContractId: parsed.data.contrato_result?.content?.id,
    payload: parsed.data,
    publicUrl,
  });
  ...
  const result = await processImaginaContractCallback(
    { db, tenant },
    parsed.data,
  );
  if (!result.success) {
