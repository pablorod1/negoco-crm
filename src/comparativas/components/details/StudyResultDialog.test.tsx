...
export const pendingResult = (
  ...
  plans: ["fijo"],
  revision: "rev-1",
  pendingSteps: ["commissions"],
  hasExistingCommissions: true,
  offerAvailable: true,
  salesCalculable: true,
  current: { sales: 0, agency: 20 },
  proposed: { sales: 12, agency: 60 },
  capabilities: {
    canResolve: true,
    canChooseType: false,
    canManualSales: true,
    commissionDecisions: ["keep", "apply", "manual"],
  },
  resolution: null,
  ...overrides,
});
...
function setup(
  ...
  const controller = {
    ...
    preview: vi.fn(),
    submit: vi.fn(),
  } satisfies StudyResultController;
  const view = render(
    <StudyResultDialog
...
describe("Study result review", () => {
  test("subcommercial cannot render the commission review dialog", () => {
    setup(pendingResult(), "2", true);

    expect(screen.queryByText(/Comisiones del plan/)).not.toBeInTheDocument();
  ...
  test("unknown type has no preselection and changing it requests read-only previews", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Selecciona el tipo de oferta" }),
    ).toBeVisible();
    ...
    rerender(
      <StudyResultDialog
        controller={{ ...controller, draft: selected }}
        role="admin"
      />,
    );
    ...
    fireEvent.click(screen.getByRole("button", { name: "Continuar" }));
    fireEvent.click(
      screen.getByRole("radio", { name: "Aplicar los importes propuestos" }),
    );
    ...
    fireEvent.click(screen.getByRole("button", { name: "Continuar" }));
    expect(
      screen.getByRole("radio", { name: /Añadir el plan/ }),
    ).not.toBeChecked();
  ...
  test("zero is an existing conflict, valid type omitted and included plan none", () => {
    ...
    expect(screen.getByRole("button", { name: "Continuar" })).toBeDisabled();
    fireEvent.click(
      screen.getByRole("radio", { name: "Mantener ambas comisiones actuales" }),
    );
    fireEvent.click(screen.getByRole("button", { name: "Continuar" }));
    fireEvent.click(
    ...
    expect(controller.submit).toHaveBeenCalledWith({
      ...
      revision: "rev-1",
      planDecision: "none",
      commissionDecision: "keep",
    });
  });

  test.each(["add", "replace"] as const)(
    "unknown missing plan can %s without an unnecessary commission step",
    (decision) => {
      const { controller } = setup(
        pendingResult({
          receivedType: null,
          ...
          pendingSteps: ["plan"],
          current: { sales: null, agency: null },
        }),
      );
      fireEvent.click(screen.getByRole("button", { name: "Continuar" }));
      fireEvent.click(
        screen.getByRole("radio", {
          name:
            decision === "add" ? /Añadir el plan/ : /Sustituir el plan actual/,
        }),
      );
      fireEvent.click(screen.getByRole("button", { name: "Continuar" }));
      fireEvent.click(
    ...
    const { controller } = setup(
      pendingResult({
        ...
        capabilities: {
          canResolve: true,
          canChooseType: false,
          canManualSales: false,
          commissionDecisions: ["keep"],
        },
      }),
    );
    expect(
      screen.getByText(/Ambas comisiones se mantienen sin cambios/),
    ).toBeVisible();
    fireEvent.click(
      screen.getByRole("radio", { name: /Sustituir el plan actual/ }),
    );
    fireEvent.click(screen.getByRole("button", { name: "Continuar" }));
    fireEvent.click(
      screen.getByRole("button", { name: "Confirmar resultado" }),
  ...
  test("role 2 sees neither agency nor manual and only allowed missing-sales decisions", () => {
    ...
    expect(
      screen.queryByRole("radio", { name: /introducir/ }),
    ).not.toBeInTheDocument();
    expect(screen.getAllByRole("radio")).toHaveLength(3);
    expect(
      screen.getByText(/No se puede calcular la comisión comercial/),
    ).toBeVisible();
    expect(
      screen.queryByRole("radio", { name: "Aplicar los importes propuestos" }),
    ).not.toBeInTheDocument();
  });
  ...
  test("empty target and incalculable sales defaults to clear without conflict step", () => {
    ...
    expect(controller.submit).toHaveBeenCalledWith(
      expect.objectContaining({ commissionDecision: "offer_clear_sales" }),
    );
  });
  ...
  test("manual zero is accepted and only manual includes manualSales", () => {
    const { controller } = setup();
    fireEvent.click(
      screen.getByRole("radio", { name: /introducir la comisión comercial/ }),
    );
    expect(screen.getByRole("button", { name: "Continuar" })).toBeDisabled();
    ...
    expect(controller.submit).toHaveBeenCalledWith(
      expect.objectContaining({ commissionDecision: "manual", manualSales: 0 }),
    );
  });
  ...
  test("cancel closes without submitting and changed revision resets confirmation", async () => {
    ...
    rerender(
      <StudyResultDialog
        controller={{
          ...controller,
          changed: true,
          draft: pendingResult({ revision: "rev-2" }),
        }}
        role="admin"
      />,
    );
    ...
    expect(
      screen.queryByRole("button", { name: "Confirmar resultado" }),
    ).not.toBeInTheDocument();
