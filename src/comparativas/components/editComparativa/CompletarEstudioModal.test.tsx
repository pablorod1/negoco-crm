...
vi.mock("@/core/hooks/use-user-company-commissions", () => ({
  useUserCompanyCommissions: mocks.rules,
}));
vi.mock("@/core/utils/sales-commission", () => ({
  calculateSalesPersonCommission: mocks.calculate,
}));
vi.mock("@/comercializadoras/hooks/useActiveEnergySuppliers", () => ({
  useActiveEnergySuppliers: mocks.suppliers,
}));
...
vi.mock("./ComissionsForm", () => ({
  default: ({
    ...
    formDataComissions: Record<string, number | null>;
  }) => (
    <div>
      {Object.entries(formDataComissions).map(([field, value]) => (
        <input key={field} aria-label={field} value={value ?? ""} readOnly />
      ))}
    </div>
  ),
}));
...
const user = {
  ...
  role: "1",
  name: "Actor",
  organization: { id: "org", name: "Negoco" },
} as User;
...
beforeEach(() => {
  ...
  mocks.suppliers.mockReturnValue({
    activeSuppliers: [{ id: "supplier", name: "Supplier" }],
  });
...
describe("AI study review values", () => {
  ...
  test("does not pick an ambiguous supplier prefix and prefers the persisted supplier", () => {
    mocks.suppliers.mockReturnValue({
      activeSuppliers: [
        { id: "one", name: "Supplier" },
        { id: "two", name: "SUPPLIER" },
        { id: "supplier", name: "Saved" },
      ],
    });
    const value = {
      ...comparison,
      ...
      abarca_estudio: {
        empresa: "Supplier - Plan",
      } as ComparativaVM["abarca_estudio"],
    };
    const view = render(
      <CompletarEstudioModal {...props} comparativa={value} />,
    );
    expect(
      screen.getByRole("button", { name: "Completar Revisión" }),
    ).toBeDisabled();
    view.rerender(
      <CompletarEstudioModal
        {...props}
        comparativa={{ ...value, company_id: "supplier" }}
      />,
    );
    expect(
      screen.getByRole("button", { name: "Completar Revisión" }),
  ...
  test.each([37, 0, null, 58])(
    "opening preserves stored decision %s despite arriving rule data",
    (sales) => {
      const value = {
        ...comparison,
        comision_sales_person: { fijo: sales, indexado: null },
      };
      const view = render(
        <CompletarEstudioModal {...props} comparativa={comparison} />,
      );
      ...
      expect(screen.getByLabelText("comision_sales_person_fijo")).toHaveValue(
        sales === null ? "" : String(sales),
      );
  ...
  test("role2 sees only sales readonly and submits no commission payload", async () => {
    ...
    expect(
      screen.queryByLabelText("comision_sales_person_fijo"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText("Comisión comercial fijo: 37 €"),
    ).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Completar Revisión" }));
    await waitFor(() => expect(fetch).toHaveBeenCalled());
    const call = vi
      .mocked(fetch)
      .mock.calls.find(([url]) => String(url).endsWith("/status"));
  ...
  test("subcommercial cannot open a study flow containing commissions", () => {
    render(
      <CompletarEstudioModal
        {...props}
        userData={{ ...user, role: "2", super_id: "manager-1" }}
    ...
    expect(screen.queryByText(/Comisión comercial/)).not.toBeInTheDocument();
  });
  ...
  test("pending decision hides completion and leaves the result action to its parent", () => {
    render(
      <CompletarEstudioModal
        {...props}
        comparativa={{ ...comparison, has_pending_study_result: true }}
      />,
    );
    expect(
    ...
    expect(
      screen.queryByRole("button", { name: "Completar Revisión" }),
    ).not.toBeInTheDocument();
  });
});
