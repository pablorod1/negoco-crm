...
vi.mock("@/crm-settings/hooks/useCrmSettings", () => ({
  useCrmSettings: () => ({ settings: { providers: [] } }),
}));
vi.mock("../ContractPreview", () => ({ default: () => null }));
vi.mock("./ContractForm", () => ({ default: () => null }));
vi.mock("@/core/hooks/use-status-badge", () => ({
  getStatusBadge: () => null,
}));
vi.mock("../InputComponent", () => ({
  InputComponent: ({ label }: { label: string }) => (
...
describe("source comparison contract wizard", () => {
  test("source zero remains readonly and can continue without empty-commission modal", () => {
    ...
    render(
      <ThirdStepForm
        ...
        tramite={{
          ...createEmptyTramiteDB(user, "fijo", comparison),
          status: "Activo",
        }}
        userData={user}
  ...
  test("subcommercial does not see copied commission values", () => {
    const subcommercial = {
      ...user,
      role: "2",
      super_id: "manager-1",
    ...
    render(
      <ThirdStepForm
        comparativa={comparison}
        tramite={{
          ...createEmptyTramiteDB(subcommercial, "fijo", comparison),
          status: "Activo",
        }}
        userData={subcommercial}
        setTramite={vi.fn()}
        contracts={[]}
        setContracts={vi.fn()}
        onBack={vi.fn()}
    ...
    expect(
      screen.queryByText("Las comisiones se copiarán desde la comparativa."),
  ...
  test("subcommercial bypasses commission confirmation for a manual request", () => {
    const submit = vi.fn();
    const subcommercial = {
      ...user,
      role: "2",
    ...
    const tramite = {
      ...createEmptyTramiteDB(subcommercial, "fijo"),
      status: "Verificado",
      comision: 50,
      comision_sales_person: 20,
    } as TramiteDB;
    ...
    expect(submit).toHaveBeenCalledOnce();
    expect(screen.queryByText("Comprobar comisiones")).not.toBeInTheDocument();
    expect(mocks.rules).toHaveBeenCalledWith(undefined);
  });
  test("Baja preview uses the source sign and restores positive amounts when leaving Baja", () => {
    const value = {
      ...
      comision_sales_person: { fijo: 30, indexado: null },
    };
    ...
    const negative = (
      setTramite.mock.calls[0][0] as (previous: TramiteDB) => TramiteDB
    )(tramite);
    expect(negative).toMatchObject({
      comision: -100,
      comision_sales_person: -30,
    });
    fireEvent.change(screen.getByLabelText("Estado"), {
      target: { value: "Activo" },
    });
    expect(setTramite.mock.calls[1][0](negative)).toMatchObject({
      comision: 100,
      comision_sales_person: 30,
    });
  });
  test("final source review displays assigned zero", () => {
    render(
      <ReviewStep
        fromComparison
        tramite={createEmptyTramiteDB(user, "fijo", comparison)}
        client={createEmptyClientDB(comparison)}
        contracts={[]}
        documents={[]}
